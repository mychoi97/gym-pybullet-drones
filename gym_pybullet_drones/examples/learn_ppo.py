...
def run(multiagent=DEFAULT_MA, output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True):
    ...
    if not multiagent:
        ...
        train_env = make_vec_env(MultiHoverAviary,
                                 env_kwargs=dict(num_drones=DEFAULT_AGENTS, num_obs = DEFAULT_NUM_OBS, num_cylinders = DEFAULT_NUM_CYLINDERS, obs=DEFAULT_OBS, act=DEFAULT_ACT, gui=DEFAULT_GUI),
                                 n_envs=1,
                                 seed=0
                                 )
        eval_env = MultiHoverAviary(num_drones=DEFAULT_AGENTS, num_obs = DEFAULT_NUM_OBS, num_cylinders = DEFAULT_NUM_CYLINDERS, obs=DEFAULT_OBS, act=DEFAULT_ACT, gui=False)
    ...
    # checkpoint_callback = CheckpointCallback(save_freq=100, save_path=checkpoint_path,
    ...
    if not multiagent:
        ...
        test_env = MultiHoverAviary(gui=gui,
                                        ...
                                        act=DEFAULT_ACT,
                                        record=record_video)
        test_env_nogui = MultiHoverAviary(num_drones=DEFAULT_AGENTS, num_obs = DEFAULT_NUM_OBS, num_cylinders = DEFAULT_NUM_CYLINDERS, obs=DEFAULT_OBS, act=DEFAULT_ACT)
    ...
    if plot and DEFAULT_OBS == ObservationType.KIN_TARGET:
        logger.plot()

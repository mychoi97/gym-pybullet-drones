...
class BaseRLAviary(BaseAviary):
    ...
    def _observationSpace(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            ############################################################
            #### OBS SPACE OF SIZE 12 (WITHOUT QUATERNION AND RPMS)
            ...
            lo = -np.inf
            hi = np.inf
            obs_lower_bound = np.array([np.repeat(np.array([lo]), 12 + self.NUM_OBS * 6 + self.NUM_CYLINDERS*3) for i in range(self.NUM_DRONES)])
            obs_upper_bound = np.array([np.repeat(np.array([hi]), 12 + self.NUM_OBS * 6 + self.NUM_CYLINDERS*3) for i in range(self.NUM_DRONES)])
            #### Add action buffer to observation space ################
            act_lo = -1
            act_hi = +1
            for i in range(self.ACTION_BUFFER_SIZE):
    ...
    def _computeObs(self):
        ...
        #### Observation vector ### 타겟과의 상대적인 위치, 속도, 가장 가까운 드론들의 상대적인 위치, 속도, 장애물과의 상대적인 위치
            ...
            # MAX_POS = 20
            MAX_POS = self.communication_range
            MAX_POS_ = 20
            target_position = self.TARGET_POS
            wall_pos = self.WALL_POS

            obs_len = 12 + self.NUM_OBS*6 + self.NUM_CYLINDERS*3
            ...
            for i in range(self.NUM_DRONES):
                ...
                distance_right_gap = pos[1] - (wall_pos[1] - self.GAP_RADIUS)
                distance_left_gap = pos[1] - (wall_pos[1] + self.GAP_RADIUS)
                distance_to_front_wall = pos[0] - wall_pos[0]
                distance_to_left_wall = pos[1] - (-4)
                distance_to_right_wall = pos[1] - 4
                distance_to_back_wall = pos[0] - (-2)
                ...
                closest_drones_info_flat = [item/MAX_POS for sublist in closest_drones_info_flat for item in sublist] 

                # 실린더 정보
                # relative_pos_cylinders = []
                # for cylinder_pos in self.cylinder_pos:
                #     relative_pos_cylinders.append(pos - cylinder_pos)

                # relative_pos_cylinders = [relative_pos_cyl/MAX_POS_ for relative_pos_cyl in relative_pos_cylinders]
                # flattened_relative_pos_cylinders = np.concatenate(relative_pos_cylinders)


                # Flatten the list of states and stack them 
                # obs_r[i, :] = np.clip(np.hstack([(target_position - pos)/MAX_POS_, vel, closest_drones_info_flat, distance_left_gap/MAX_POS_, distance_right_gap/MAX_POS_, distance_to_front_wall/MAX_POS_, distance_to_left_wall/MAX_POS_, distance_to_right_wall/MAX_POS_, distance_to_back_wall/MAX_POS_, flattened_relative_pos_cylinders]).reshape(obs_len,), -1, 1)
                obs_r[i, :] = np.clip(np.hstack([(target_position - pos)/MAX_POS_, vel, closest_drones_info_flat, distance_left_gap/MAX_POS_, distance_right_gap/MAX_POS_, distance_to_front_wall/MAX_POS_, distance_to_left_wall/MAX_POS_, distance_to_right_wall/MAX_POS_, distance_to_back_wall/MAX_POS_]).reshape(obs_len,), -1, 1)
